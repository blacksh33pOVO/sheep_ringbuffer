...
int buffer_add(buffer_t *r, const void *data, uint32_t sz)
{
    if (sz > rb_remain(r))
        return -1;
    uint32_t i;
    //(r->tail & (r->size - 1)) == r->tail % r->size
    i = min(sz, r->size - (r->tail & (r->size - 1)));
    memcpy(r->buf + r->size - (r->tail & (r->size - 1)), data, i);
...
int buffer_search(buffer_t *r, const char* sep, const int seplen)
{
    ...
    for ( i = 0; i < rb_len(r) - seplen; i++)
    {
        ...
        if (pos + seplen > r->size)
        {
            ...
            if (memcmp(r->buf, sep + r->size - pos, seplen - r->size + pos) == 0)
            {
                return i + seplen;
            }
...
uint8_t * buffer_write_atmost(buffer_t *r)
{
    ...
    if (wpos < rpos)
    {
        ...
        memcpy(tmp, r->buf + rpos, r->size - rpos);
        memcpy(tmp + r->size - rpos, r->buf, wpos);
